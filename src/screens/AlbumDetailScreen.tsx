...
const AlbumDetailScreen = () => {
  ...
  const playNextInQueue = async () => {
    try {
      console.log('[AlbumDetailScreen] 🎵 playNextInQueue called');
      console.log('[AlbumDetailScreen] 🎵 Current queue length:', queue.length);
      console.log('[AlbumDetailScreen] 🎵 Current index:', currentIndex);
      console.log('[AlbumDetailScreen] 🔁 Repeat mode:', repeatMode);
      
      // Si repeatMode es 'one', repetir la misma canción
      if (repeatMode === 'one' && currentTrack) {
        console.log('[AlbumDetailScreen] 🔁 Repitiendo canción actual');
        ...
        if (sound) {
          try {
            ...
            console.log('[AlbumDetailScreen] Sound ya detenido');
          }
          setSound(null);
        }
      ...
      if (currentIndex < queue.length - 1) {
        ...
        console.log('[AlbumDetailScreen] 🎵 Playing next track:', nextTrack.title, `(${nextIndex + 1}/${queue.length})`);
        ...
        if (!localPath) {
          console.log('[AlbumDetailScreen] ❌ No localPath, saltando');
        ...
        console.log('[AlbumDetailScreen] 🔁 Repeat all: volviendo al inicio');
        ...
        if (!localPath) {
          console.log('[AlbumDetailScreen] ❌ No localPath para primer track');
          setIsPlaying(false);
          return;
        }
        ...
        console.log('[AlbumDetailScreen] 🎵 Queue finished');
        setIsPlaying(false);
      }
    } catch (error) {
      console.error('[AlbumDetailScreen] ❌ Error in playNextInQueue:', error);
      setIsPlaying(false);
    }
  };
